...
class Overlay:
    ...
    def __init__(self, configfile: str):
        ...
        if not path.exists(configfile):
            ...
            self.fixConfig(open(configfile, 'r').read(), configfile)
        ...
        try:
            ...
            err_helper.showError('0x7', e, crash=True)
    ...
    def onNewLine(self, line):
        ...
        if 'Your new API key is' in line:
            self.api = line[len('[02:39:43] [Client thread/INFO]: [CHAT] Your new API key is'):].strip()
    ...
    def check_api(self):
        try:
            ...
            baseURL = 'https://api.hypixel.net/player?key='
            hyinfo = requests.get(f'{baseURL}{self.config["Api_Key"].strip()}&name={self.config["IGN"].strip()}')
            ...
            if hyinfo.status_code in range(200, 299):
                ...
                if 'recently' in hyinfo_data.get('cause') and hyinfo.status_code == 429:
                    ...
                    if hyinfo.status_code not in range(200, 299):
                        ...
                        logging.info('Successfully Checked Used Second Hand')


        except KeyError as e:
            err_helper.showError('0x7', e)
    ...
    def editWhitelist(self):
        ...
        while True:
            ...
            try:
                ...
                logging.warning('\033[33mYou Call That A NUMBER??')
        if opt == 1:
            while True:
                try:
                    ...
                    self.fixConfig(json.dumps(self.config), './config.json')
        elif opt == 2:
            while True:
                try:
    ...
    def fixConfig(self, config_str, fp):
        ...
        logging.warning('\033[33mError while loading Config, Trying to Fix')
        if defaults.CONFIG.keys() & json.loads(config_str).keys() != defaults.CONFIG.keys():
    ...
    def loadConfig(self, fp: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                err_helper.showError('0x7', crash=True)
